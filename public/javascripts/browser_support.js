...
var BrowserSupport = function() {
    this.popupElement = $( '#browser_support' );
    this.supportedBrowsers = [ "chrome", "safari", "firefox" ];
    this.cookieString = "dismissed_browser_popup";
    ...
    this.isBrowserSupported = function() {
        if( this.supportedBrowsers.indexOf( this.browserType() ) === -1 || ( this.browserType() === "safari" && this.osType() !== "mac" )) {
            return false
        } else {
            return true
        }
    ...
    this.checkBrowser = function() {
        if( !this.isBrowserSupported() || this.checkMobile() ){
            this.openPopup();
        }
    };
    ...
    this.openPopup = function() {
        if( document.cookie.indexOf( this.cookieString ) === -1 ){
    ...
    this.dismissPopup = function() {
        ...
        document.cookie = "browser_support=" + this.cookieString;
    };

    this.checkBrowser();

    this.popupElement.bind( 'click', _.bind( function() {
        this.dismissPopup();
    }, this ) );
};
