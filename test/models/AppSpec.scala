...
class AppSpec extends SpecificationWithFixtures {
  ...
  "App.findAppWithVirtualCurrency" should {
    "return an instance of the AppWithVirtualCurrency class" in new WithAppDB(distributor.id.get) {
      appService.findAppWithVirtualCurrency(currentApp.id, distributor.id.get).get must haveClass[AppWithVirtualCurrency]
    }

    "return attributes for both App and VirtualCurrency" in new WithAppDB(distributor.id.get) {
      val appWithCurrency = appService.findAppWithVirtualCurrency(currentApp.id, distributor.id.get).get
  ...
  "App.findAllAppsWithWaterfalls" should {
    "return a list of Apps containing the Waterfall ID for each App" in new WithDB {
      ...
      val appID = appService.create(newDistributor.id.get, "New App", testPlatform.Ios.PlatformID).get
      val waterfallID = database.withTransaction { implicit connection => waterfallService.create(appID, "New Waterfall") }.get
      val appWithWaterfallID = appService.findAllAppsWithWaterfalls(newDistributor.id.get).head
    ...
    "return an empty list if no Apps could be found" in new WithDB {
      ...
      appService.findAllAppsWithWaterfalls(fakeDistributorID).size must beEqualTo(0)
    }
  }
  ...
  "App.findAppWithWaterfalls" should {
    "return an App containing the Waterfall ID for the App" in new WithAppDB(distributor.id.get) {
      val appWithWaterfallID = appService.findAppWithWaterfalls(currentApp.id, distributor.id.get).get
    ...
    "return an empty list if no Apps could be found" in new WithDB {
      val fakeAppID = 12345
      appService.findAppWithWaterfalls(fakeAppID, distributor.id.get) must beNone
    }
  }

  "App.create" should {
    "properly save a new App in the database" in new WithDB {
      val appID = appService.create(distributor.id.get, appName, testPlatform.Ios.PlatformID).get
      appService.find(appID).get.name must beEqualTo(appName)
    }
    ...
    "should set the app_config_refresh_interval to 0 by default" in new WithDB {
      ...
      val appConfigRefreshInterval = database.withConnection { implicit connection =>
        SQL(
          """
             SELECT app_config_refresh_interval FROM apps WHERE id={id};
          """
        ).on("id" -> appID).as(SqlParser.long("app_config_refresh_interval").single)
      }
      appConfigRefreshInterval must beEqualTo(0)
    }
  }
  ...
  "App.findAll" should {
    "return a list of all apps associated with the Distributor ID" in new WithDB {
      ...
      appService.create(currentDistributor.id.get, "App 1", testPlatform.Ios.PlatformID)
      appService.create(currentDistributor.id.get, "App 2", testPlatform.Ios.PlatformID)
      val apps = appService.findAll(currentDistributor.id.get)
      apps.size must beEqualTo(2)
    }

    "return an empty list if the Distributor ID is not found" in new WithDB {
      val fakeDistributorID = 12345
      appService.findAll(fakeDistributorID).size must beEqualTo(0)
    }
  }

  "App.find" should {
    "return an instance of the App class if the ID is found" in new WithAppDB(distributor.id.get) {
      appService.find(currentApp.id).get must haveClass[App]
    }

    "return None if the App ID is not found" in new WithDB {
      val fakeAppID = 12345
      appService.find(fakeAppID) must beNone
    }
  }

  "App.findWithTransaction" should {
    "return an instance of the App class if the ID is found" in new WithAppDB(distributor.id.get) {
  ...
  "App.findByWaterfallID" should {
    "return an instance of the App class if the Waterfall ID is found" in new WithAppDB(distributor.id.get) {
      appService.findAppByWaterfallID(currentWaterfall.id).get must beEqualTo(currentApp)
    }

    "return None if the Waterfall ID is not found" in new WithDB {
      val fakeWaterfallID = 12345
      appService.findAppByWaterfallID(fakeWaterfallID) must beNone
    }
  }
  ...
  "App.update" should {
    "update the field(s) for a given App" in new WithDB {
      ...
      appService.update(updatedAppClass)
      val updatedApp = appService.find(app1.id).get
  ...
  "App.updateAppConfigRefreshInterval" should {
    "update the appConfigRefreshInterval attribute for an App" in new WithAppDB(distributor.id.get) {
      ...
      waterfallAdProviderService.create(currentWaterfall.id, adProviderID1.get, None, Some(5.0), true, true)
      ...
      val originalConfig = appConfigService.findLatest(currentApp.token).get
      ...
      appService.updateAppConfigRefreshInterval(currentApp.id, newAppConfigRefreshInterval) must beEqualTo(true)
    ...
    "not update the appConfigRefreshInterval if the App is in test mode" in new WithAppDB(distributor.id.get) {
      ...
      database.withTransaction { implicit connection => appConfigService.createWithWaterfallIDInTransaction(currentWaterfall.id, None) }
      val originalConfig = appConfigService.findLatest(currentApp.token).get
      appService.updateAppConfigRefreshInterval(currentApp.id, 500) must beEqualTo(false)
      val latestConfig = appConfigService.findLatest(currentApp.token).get
    ...
    "return false if the App ID is not found" in new WithDB {
      val fakeAppID = 12345
      appService.updateAppConfigRefreshInterval(fakeAppID, 500) must beEqualTo(false)
    }
  }
}
