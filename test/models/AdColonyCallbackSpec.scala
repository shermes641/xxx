...
class AdColonyCallbackSpec extends SpecificationWithFixtures with AdProviderSpecSetup {
  ...
  running(FakeApplication(additionalConfiguration = testDB)) {
    val id = waterfallAdProviderService.create(waterfall.id, adColonyID, None, None, true, true).get
  ...
  val goodCallback = running(FakeApplication(additionalConfiguration = testDB)) {
    new AdColonyCallback(appToken, transactionID, uid, amount, currency, openUDID, udid, odin1, macSha1, verifier, customID, waterfallAdProviderService)
  }

  "adProviderName" should {
    "be set when creating a new instance of the AdColonyCallback class" in new WithDB {
      val callback = new AdColonyCallback(appToken, transactionID, uid, amount, currency, openUDID, udid, odin1, macSha1, verifier, customID, waterfallAdProviderService)
  ...
  "verificationInfo" should {
    ...
    "not be valid when the generated verification does not match the received verification string" in new WithDB {
      ...
      val callback = new AdColonyCallback(appToken, transactionID, uid, amount, currency, openUDID, udid, odin1, macSha1, invalidVerifier, customID, waterfallAdProviderService)
      val verification = callback.verificationInfo
