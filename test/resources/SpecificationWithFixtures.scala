...
abstract class SpecificationWithFixtures extends Specification {
  ...
  def insertData =
    DB.withConnection { implicit c =>
      ...
      SQL("INSERT INTO Distributor (name) VALUES ('TestDistributor')").executeInsert()

      SQL("INSERT INTO DistributorUser (email, hashed_password, distributor_id) VALUES ({email}, {pwd}, 1)")
        .on("pwd" -> hashedPassword, "email" -> email).executeInsert()

      SQL("INSERT INTO Property (name, distributor_id) VALUES ('TestProperty', 1)").executeInsert()
