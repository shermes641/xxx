...
if (Environment.isProd) {
  ...
  class runImport extends ConfigVars {
    ...
    object AppHelper extends UpdateHyprMarketplace {
      def createApp(appName: String, platform: Platform) = {
        ...
        val wap = DB.withTransaction { implicit connection =>
          ...
          ConfigVarsApp.create(appID, app.token, 0)
    ...
    class DailyEvent(appName: String, appID: Long, adProvider: AdProvider, startDate: DateTime) extends ConfigVars {

      val client = new Client(
        projectId = ConfigVarsKeen.projectID,
        masterKey = ConfigVarsKeen.masterKey,
        writeKey = ConfigVarsKeen.writeKey,
        readKey = ConfigVarsKeen.readKey
      )
    ...
    val keenCountURLRoot = Constants.KeenConfig.ProjectsUrl +
      ConfigVarsKeen.projectID +
      "/queries/count?api_key=" +
      ConfigVarsKeen.readKey +
      "&filters=%5B%7B%22property_name%22%3A%22distributor_id%22%2C%22operator%22%3A%22in%22%2C%22property_value%22%3A%5B" +
      distributorID.toString +
      "%5D%7D%5D"
    val keenAverageURLRoot = Constants.KeenConfig.ProjectsUrl +
      ConfigVarsKeen.projectID +
      "/queries/average?api_key=" +
      ConfigVarsKeen.readKey +
      "&filters=%5B%7B%22property_name%22%3A%22distributor_id%22%2C%22operator%22%3A%22in%22%2C%22property_value%22%3A%5B" +
      distributorID.toString +
      "%5D%7D%5D"
