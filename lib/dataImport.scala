...
if(AdProvider.findAll.size == 0) {
  ...
  (adColonyResult, hyprResult, vungleResult, appLovinResult) match {
    case (Some(adColonyID), Some(hyprID), Some(vungleID), Some(appLovinID)) => println("All ad providers created successfully!")
    case (_, _, _, _) => println("Something went wrong and one or more of the ad providers were not created properly.")
  }
} else {
  println("Running this script without an empty ad_providers table may result in duplicate ad providers.  If you need to create one or more of these ad providers, you should do it manually.")
}
