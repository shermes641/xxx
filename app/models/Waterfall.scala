...
object Waterfall {
  ...
  def withWaterfallAdProviders(id: Long): Map[Waterfall, List[models.WaterfallAdProvider]] = {
    DB.withConnection { implicit c =>
      val query = SQL(
        """
          SELECT waterfalls.*, waterfall_ad_providers.*
          FROM waterfalls JOIN waterfall_ad_providers on waterfall_ad_providers.waterfall_id = waterfalls.id
